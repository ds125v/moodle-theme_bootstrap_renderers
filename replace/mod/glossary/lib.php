...
function glossary_print_alphabet_menu($cm, $glossary, $mode, $hook, $sortkey='', $sortorder = '') {
    if ( $mode != 'date' ) {
        ...
        echo '<ul class="pagination pagination-mini">';
        glossary_print_special_links($cm, $glossary, $mode, $hook);
        glossary_print_alphabet_links($cm, $glossary, $mode, $hook, $sortkey, $sortorder);
        glossary_print_all_links($cm, $glossary, $mode, $hook);
        echo '</ul>';
...
function glossary_print_author_menu($cm, $glossary,$mode, $hook, $sortkey = '', $sortorder = '') {
    if ($glossary->showalphabet) {
        echo '<div class="glossaryexplain">' . get_string("explainalphabet","glossary") . '</div>';
    }
...
global $CFG;
     if ( $glossary->showall) {
         ...
         if ( $hook == 'ALL' ) {
              echo "<li class=active><span>$strallentries</span></li>";
         } else {
              $strexplainall = strip_tags(get_string("explainall","glossary"));
              echo "<li><a title=\"$strexplainall\" href=\"$CFG->wwwroot/mod/glossary/view.php?id=$cm->id&amp;mode=$mode&amp;hook=ALL\">$strallentries</a></li>";
         }
     }
}
...
global $CFG;
     if ( $glossary->showspecial) {
         ...
         if ( $hook == 'SPECIAL' ) {
              echo "<li class=active><span>$strspecial</span></li>";
         } else {
              $strexplainspecial = strip_tags(get_string("explainspecial","glossary"));
              echo "<li><a title=\"$strexplainspecial\" href=\"$CFG->wwwroot/mod/glossary/view.php?id=$cm->id&amp;mode=$mode&amp;hook=SPECIAL\">$strspecial</a></li>";
         }
     }
}
     ...
     if ( $glossary->showalphabet) {
          ...
          for ($i = 0; $i < count($alphabet); $i++) {
              if ( $hook == $alphabet[$i] and $hook) {
                   echo "<li class=active><span>$alphabet[$i]<span></li>";
              } else {
                   echo "<li><a href=\"$CFG->wwwroot/mod/glossary/view.php?id=$cm->id&amp;mode=$mode&amp;hook=".urlencode($alphabet[$i])."&amp;sortkey=$sortkey&amp;sortorder=$sortorder\">$alphabet[$i]</a></li>";
              }
          }
     }
}
